    ...
    override fun collectObjectsFromPoint(
        ...
        for (piloto in pilotos) {
            ...
            if (esLocal) {
                ...
                val radiusSq = (radius * 1.5f) * (radius * 1.5f)

                if (touchDistSq <= radiusSq || tileBox.isLatLonNearPixel(piloto.lat, piloto.lon, point.x, point.y, radius)) {
                    pilotoSeleccionado?.invoke(piloto)
                    result.collect(piloto, this)
                }
            } else {
                if (tileBox.isLatLonNearPixel(piloto.lat, piloto.lon, point.x, point.y, radius)) {
                    pilotoSeleccionado?.invoke(piloto)
                    result.collect(piloto, this)
                }
            }
